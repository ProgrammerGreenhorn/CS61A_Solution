...
def subseqs(s):
    ...
    if not s:
       ...
       first , rest = s[0] , s[1:]
       return insert_into_all(first,subseqs(rest)) + subseqs(rest)
...
def make_glookup(class_assignments):
    ...
    score_sofar = 0
    total_score_sofar = 0
    def glookup(section,score):
        nonlocal score_sofar,total_score_sofar
        score_sofar += score
        total_score_sofar += class_assignments[section]
        return score_sofar/total_score_sofar
